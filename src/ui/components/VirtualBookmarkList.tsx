...
const ITEM_HEIGHT = 120; // 每个书签项的高度
const BUFFER_SIZE = 5; // 缓冲区大小

export const VirtualBookmarkList: React.FC<VirtualBookmarkListProps> = ({
  bookmarks,
  selectedBookmarks,
  onSelectionChange,
  onRefresh,
  ...
  const visibleRange = useMemo(() => {
    if (filteredAndSortedBookmarks.length <= 50) {
      // 少于50项时不使用虚拟滚动
      return {
        start: 0,
        end: filteredAndSortedBookmarks.length,
        useVirtual: false
      };
    }
  ...
  }, [scrollTop, containerHeight, filteredAndSortedBookmarks.length]);
  ...
  const visibleBookmarks = useMemo(() => {
    ...
    return filteredAndSortedBookmarks.slice(visibleRange.start, visibleRange.end);
  }, [filteredAndSortedBookmarks, visibleRange]);
  ...
  const totalHeight = filteredAndSortedBookmarks.length * ITEM_HEIGHT;
  const offsetY = visibleRange.useVirtual ? visibleRange.start * ITEM_HEIGHT : 0;
              ...
              <div style={{ height: totalHeight, position: 'relative' }}>
                ...
                >
                  {visibleBookmarks.map((bookmark, index) => {
                    const actualIndex = visibleRange.useVirtual ? visibleRange.start + index : index;
                    return (
                      <BookmarkItem
                        key={`${bookmark.sourceId}-${actualIndex}`}
                        bookmark={bookmark}
